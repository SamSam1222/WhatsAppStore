...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your account has been created successfully! You are now logged in.')  # Success message
            return redirect('index')  # Redirect to the desired page (like home or dashboard)
        else:
            # Form is invalid, return the form with errors
            messages.error(request, 'There was an error with your signup, please check the form and try again.')  # Error message
